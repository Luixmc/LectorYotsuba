	...
	*/
	...
	$(document).keydown(function(e){
		...
		if( e.keyCode == 37 && $('.wp-manga-nav .nav-links .nav-previous > a').length > 0 ){	//left key
			$($('.wp-manga-nav .nav-links a.prev_page')[0]).trigger('click');
		}else if( e.keyCode == 39 &&  $('.wp-manga-nav .nav-links .nav-next > a').length > 0 ){ //right key
			$($('.wp-manga-nav .nav-links a.next_page')[0]).trigger('click');
		}
	});
	...
	$('input[name="wp-manga-user-avatar"]').on('change', function(e){
		$('.file-name').html($('input[name="wp-manga-user-avatar"]')[0].files[0].name);
	});
